...
// Define the type for the status keys
type StatusKey = keyof typeof STATUS_CONTENT_MAP;
...
export default function CompletePage() {
  ...
  useEffect(() => {
    ...
    stripe.retrievePaymentIntent(clientSecret).then(({ paymentIntent }) => {
      ...
      // Ensure the status is a valid key in STATUS_CONTENT_MAP
      const validStatus: StatusKey = STATUS_CONTENT_MAP.hasOwnProperty(paymentIntent.status)
        ? (paymentIntent.status as StatusKey)
        : "default";
  ...
  return (
    <div className={styles.paymentStatus}>
      ...
      <h2 className={styles.statusText}>{STATUS_CONTENT_MAP[status].text}</h2>
      {intentId && (
          ...
          <table className={styles.table}>
            <tbody>
              <tr>
                ...
                <td className={styles.tableContent}>{intentId}</td>
              </tr>
              <tr>
                <td className={styles.tableLabel}>status</td>
                <td className={styles.tableContent}>{status}</td>
              </tr>
            </tbody>
          </table>
        </div>
      ...
      <a className={styles.retryButton} href="/checkout">
        Test another
      </a>
    </div>
  );
